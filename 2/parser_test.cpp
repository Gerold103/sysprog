...
test_one_word(void)
{
	...
	p.feed("ls\n", 3);
	unit_check(p.pop_next(line) == PARSER_ERR_NONE, "parse");
	unit_check(line->out_type == OUTPUT_TYPE_STDOUT, "out type");
	unit_check(!line->out_file, "out file");
	unit_check(!line->is_background, "is background");
	unit_check(line->exprs.size() == 1, "expr count");
	...
	unit_check(p.pop_next(line) == PARSER_ERR_NONE, "parse");
	unit_check(line == nullptr, "no more lines yet");
...
test_two_words(void)
{
	...
	for (uint32_t i = 0; i < len; ++i) {
		p.feed(&str[i], 1);
		unit_fail_if(p.pop_next(line) != PARSER_ERR_NONE);
		unit_fail_if(line != nullptr);
	}
	str = "   \t\r  \n";
	p.feed(str, strlen(str));
	unit_check(p.pop_next(line) == PARSER_ERR_NONE, "parse");
	unit_check(line->exprs.size() == 1, "expr count");
	expr *e = &line->exprs.front();
	unit_check(e->type == EXPR_TYPE_COMMAND, "expr type");
...
test_escape_outside_of_string(void)
{
	...
	unit_check(line->out_type == OUTPUT_TYPE_STDOUT, "out type");
	unit_check(!line->is_background, "not background");
	unit_check(line->exprs.size() == 3, "expr count");
	std::list<expr>::iterator it = line->exprs.begin();
	e = &*it;
	unit_check(e->type == EXPR_TYPE_COMMAND, "expr type");
	unit_check(e->cmd.exe == "echo", "exe");
	unit_check(e->cmd.args.size() == 1, "arg count");
...
test_pipe(void)
{
	...
	unit_check(line->out_type == OUTPUT_TYPE_STDOUT, "out type");
	unit_check(!line->is_background, "not background");
	unit_check(line->exprs.size() == 5, "expr count");
	it = line->exprs.begin();
	e = &*it;
	unit_check(e->type == EXPR_TYPE_COMMAND, "expr type");
	unit_check(e->cmd.exe == "echo", "exe");
	...
	unit_check(line->out_type == OUTPUT_TYPE_STDOUT, "out type");
	unit_check(!line->is_background, "not background");
	unit_check(line->exprs.size() == 7, "expr count");
	it = line->exprs.begin();
	e = &*it;
	unit_check(e->type == EXPR_TYPE_COMMAND, "expr type");
...
test_comments(void)
{
	...
	p.feed("\n", 1);
	unit_check(p.pop_next(line) == PARSER_ERR_NONE, "parse");
	unit_check(line == nullptr, "comment lines are skipped");
...
test_logical_operators(void)
{
	...
	unit_check(e->cmd.args.size() == 0, "arg count");
...
static void
test_error_one(parser p, const char *expr, parser_error err)
{
	command_line *line = nullptr;
	...
	p.feed("\n", 1);
	unit_check(p.pop_next(line) == err, "parse error");
	unit_check(line == nullptr, "no line");
	unit_check(p.pop_next(line) == PARSER_ERR_NONE, "parse none");
}
...
test_errors(void)
{
	...
	p.feed("echo\n", 5);
	unit_check(p.pop_next(line) == PARSER_ERR_NONE, "parse ok");
	unit_check(line->exprs.size() == 1, "expr count");
	unit_check(line->exprs.front().type == EXPR_TYPE_COMMAND, "expr type");
	unit_check(line->exprs.front().cmd.exe == "echo", "exe");
	delete line;
